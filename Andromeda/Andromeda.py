# andromeda.py – Andromeda v0.1 Core
import numpy as np
...
class AndromedaBlock:
    def __init__(self, dim=64, n_heads=4):
        ...
        # Random weights (later trainbaar)
...
if __name__ == "__main__":
    np.random.seed(42)
    block = AndromedaBlock()
    x = np.random.randn(1, 8, 64)
    mask = np.triu(np.full((1, 1, 8, 8), -1e9), k=1)
    out = block.forward(x, mask)
    print("ANDROMEDA BLOCK WERKT!")
